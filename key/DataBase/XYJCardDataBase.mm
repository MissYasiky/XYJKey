...
- (NSArray<XYJCard *> *)getAllData {
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        for (XYJCard *data in dataArray) {
            [[XYJCardDataBase sharedDataBase] updataData:data];
        }
    });
...
- (void)updataData:(XYJCard *)data {
    if (data.externDict != nil) {
        ...
        BOOL success = [self.database updateRowsInTable:kCardDataBaseTableName onProperty:XYJCard.externString withObject:data where:XYJCard.createTime == data.createTime];
        if (!success) {
